...
while (true) {
    ...
    mlog("??  Vídeo #$video_id — \"$title\" — path: $video_path");
    ...
    mlog("   ??  A enviar pasta HLS para o R2 como \"$unique_name\"...");
    ...
    mlog("   ???  A apagar MP4 original do R2...");
    $deleted = r2_delete_video($video_path);
    if (!$deleted) {
        mlog("   ??  Aviso: não foi possível apagar o MP4 original — mas o vídeo foi migrado na mesma.");
    }
