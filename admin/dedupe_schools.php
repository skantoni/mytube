...
for ($i = 0; $i < count($keys); $i++) {
    ...
    for ($j = $i + 1; $j < count($keys); $j++) {
        ...
        $dist = levenshtein_normalized($keys[$i], $keys[$j]);
    ...
    if (!isset($_POST['csrf_token']) || !csrf_verify()) {
        ...
        if ($keep_id && !empty($remove_ids)) {
            try {
                ...
                $valid_check->execute($all_ids);

                if ((int)$valid_check->fetchColumn() !== count($all_ids)) {
                ...
                $stmt = $pdo->prepare("UPDATE users SET school_id = ? WHERE school_id IN ($placeholders_remove)");
                ...
                foreach ($remove_ids as $rid) {
                    ...
                    $pdo->prepare("UPDATE best_mytuber_candidates SET school_id = ? WHERE school_id = ?")
                        ->execute([$keep_id, $rid]);
                    $pdo->prepare("UPDATE best_mytuber_weekly SET school_id = ? WHERE school_id = ?")
                        ->execute([$keep_id, $rid]);
                }
